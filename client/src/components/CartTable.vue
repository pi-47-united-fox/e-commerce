<template>
    <tr>
        ...
        <th scope="row">{{ cart.Product.category }}</th>
        ...
        <td>
            ...
            <b-icon icon="clipboard-minus" style="color: black;" aria-hidden="true" disabled></b-icon>
            </b-button>
            &nbsp;
            {{ cart.quantity }}
            &nbsp;
            <b-button class="mb-2 icon-cart" variant="outline-white" scale="1.1" @click.prevent="willPlus(cart.id, cart.quantity, cart.Product.price)">
              <b-icon icon="clipboard-plus" style="color: black;" aria-hidden="true" disabled></b-icon>
            </b-button>
        </td>
...
export default {
  ...
  computed: {
    ...
    stock () {
      return this.cart.Product.stock - this.cart.quantity
    }
  },
  methods: {
    ...mapMutations([
    ...
    willPlus (id, currentQuantity, price) {
      if (currentQuantity >= this.cart.Product.stock) {
        return false
      } else {
        this.updateQuantity({
          id,
